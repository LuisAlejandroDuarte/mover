...
class CredencialEmpresaScreen extends ConsumerStatefulWidget {
  const CredencialEmpresaScreen({super.key});

  @override
  ConsumerState<CredencialEmpresaScreen> createState() =>
      _CredencialEmpresaScreenState();
}

class _CredencialEmpresaScreenState
    extends ConsumerState<CredencialEmpresaScreen> {
  User? user;
  ...
  Future<void> createEmpresa() async {
    ...
    try {
      ...
      if (user!.id == 0) {
        ...
        Empresa? empresa = await empresaRepository.crearEmpresa(user!.empresa);

        User? userNew = user!.copyWith(empresa: empresa);
