...
class OrigenScreen extends ConsumerStatefulWidget {
  final Function() onOrigenScreenSelected;
  const OrigenScreen({required this.onOrigenScreenSelected, super.key});
...
class _OrigenScreenState extends ConsumerState<OrigenScreen> {
  ...
  void saveOrigen() async {
    oferta = await ofertaStorage.get();
    ubicacion = await ubicacionOrigenStorage.get();

    await ubicacionOrigenStorage
  ...
  void _showSaveDialog(maps.LatLng latLng, String direccion,
    ...
    ubicacionOrigenStorage.save(Ubicacion(
      ciudadId: ciudad!.id,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Seleccione el origen'),
      ),
      body: Stack(
        children: [
          Column(
